...
var PartialEvaluator = (function PartialEvaluatorClosure() {
    ...
    PartialEvaluator.prototype = {
        ...
        buildFormXObject: function PartialEvaluator_buildFormXObject(resources, xobj, smask, operatorList) {
            var self = this;
        ...
        buildPaintImageXObject: function PartialEvaluator_buildPaintImageXObject(resources, image, inline, operatorList) {
            var self = this;
            var dict = image.dict;
            ...
            PDFImage.buildImage(function (imageObj) {
                var imgData = imageObj.getImageData();
                self.handler.send('obj', [objId, self.pageIndex, 'Image', imgData], null, [imgData.data.buffer]);
            }, self.handler, self.xref, resources, image, inline);
            operatorList.addOp(OPS.paintImageXObject, args);
        },
        handleTilingType: function PartialEvaluator_handleTilingType(fn, args, resources, pattern, patternDict, operatorList) {
            // Create an IR of the pattern code.
            var tilingOpList = this.getOperatorList(pattern, patternDict.get('Resources') || resources);
        ...
        handleSetFont: function PartialEvaluator_handleSetFont(resources, fontArgs, fontRef, operatorList) {
            ...
            var self = this;
            var font = this.loadFont(fontName, fontRef, this.xref, resources, operatorList);
        ...
        setGState: function PartialEvaluator_setGState(resources, gState, operatorList) {
            var self = this;
            ...
            function setGStateForKey(gStateObj, key, value) {
                switch (key) {
                    ...
                    case 'ca':
                        ...
                        break;
                    case 'Font':
                        var loadedName = self.handleSetFont(resources, null, value[0], operatorList);
        ...
        loadFont: function PartialEvaluator_loadFont(fontName, font, xref, resources, parentOperatorList) {
            function errorFont() {
                return {
        ...
        getOperatorList: function PartialEvaluator_getOperatorList(stream, resources, operatorList) {
            var self = this;
            var xref = this.xref;
                ...
                if (isCmd(obj)) {
                    ...
                    switch (fn) {
                        ...
                        case OPS.setFillColorN:
                            ...
                            if (isName(patternName) &&
                                ...
                                if (typeNum == TILING_PATTERN) {
                                    self.handleTilingType(fn, args, resources, pattern, dict, operatorList);
                                    args = [];
                                    continue;
                                }
                                    ...
                                    var pattern = Pattern.parseShading(shading, matrix, xref, resources);
                                    args = pattern.getIR();
                                }
                                else {
                        ...
                        case OPS.paintXObject:
                            ...
                            if (xobj) {
                                ...
                                if ('Form' == type.name) {
                                    ...
                                    self.buildPaintImageXObject(resources, xobj, false, operatorList);
                                    args = [];
                                    continue;
                                }
                                else {
                        ...
                        case OPS.shadingFill:
                            ...
                            var shadingFill = Pattern.parseShading(shading, null, xref, resources);
        ...
        getTextContent: function PartialEvaluator_getTextContent(stream, resources, state) {
            var bidiTexts;
                ...
                if (isCmd(obj)) {
                    ...
                    switch (cmd) {
                        ...
                        case 'BT':
                            ...
                            break;
                        case 'cm':
                            textState.transformCTM(args[0], args[1], args[2], args[3], args[4], args[5]);
                            break;
                        case 'TJ':
                            var items = args[0];
                        ...
                        case 'Do':
                            ...
                            if ('Form' !== type.name)
                                break;
                            state = this.getTextContent(xobj, xobj.dict.get('Resources') || resources, state);
                            break;
                        case 'gs':
                            var dictName = args[0];
                            var extGState = resources.get('ExtGState');
        ...
        translateFont: function PartialEvaluator_translateFont(dict, xref) {
            ...
            if (!descriptor) {
                if (type.name == 'Type3') {
                    ...
                    // Simulating descriptor flags attribute
                    var fontNameWoStyle = baseFontName.split('-')[0];
                    var flags = (this.isSerifFont(fontNameWoStyle) ? FontFlags.Serif : 0) |
        ...
        function PartialEvaluator_optimizeQueue(queue) {
            ...
            for (var i = 0, ii = argsArray.length; i < ii; i++) {
                if (fnArray[i] === OPS.paintInlineImageXObject &&
                    ...
                    argsArray.splice(j, count * 4, [{ width: imgWidth, height: imgHeight, data: imgData }, map]);
                    i = j;
                    ii = argsArray.length;
                }
            }
