...
var PDFImage = (function PDFImageClosure() {
    ...
    PDFImage.buildImage = function PDFImage_buildImage(callback, handler, xref, res, image, inline) {
        ...
        // resolved.
        Promise.all([imageDataPromise, smaskPromise, maskPromise]).then(function (results) {
            var imageData = results[0], smaskData = results[1], maskData = results[2];
            var image = new PDFImage(xref, res, imageData, inline, smaskData, maskData);
            callback(image);
        });
    ...
    PDFImage.createMask = function PDFImage_createMask(imgArray, width, height, inverseDecode) {
        try {
    ...
    PDFImage.prototype = {
        ...
        fillRgbaBuffer: function PDFImage_fillRgbaBuffer(buffer, width, height) {
            ...
            var rgbBuf = this.colorSpace.createRgbBuffer(comps, 0, originalWidth * originalHeight, bpc);
            if (originalWidth != width || originalHeight != height)
                rgbBuf = PDFImage.resize(rgbBuf, this.bpc, 3, originalWidth, originalHeight, width, height);
            var compsPos = 0;
            var opacityPos = 0;
