...
var JpxImage = (function JpxImageClosure() {
    ...
    function copyCoefficients(coefficients, x0, y0, width, height, delta, mb, codeblocks, transformation, segmentationSymbolUsed) {
        ...
        for (var i = 0, ii = codeblocks.length; i < ii; ++i) {
            ...
            bitModel = new BitModel(blockWidth, blockHeight, codeblock.subbandType, codeblock.zeroBitPlanes);
    ...
    function transformTile(context, tile, c) {
        ...
        for (var i = 0; i <= decompositionLevelsCount; i++) {
            ...
            for (var j = 0, jj = resolution.subbands.length; j < jj; j++) {
                ...
                copyCoefficients(coefficients, subband.tbx0, subband.tby0, width, height, delta, mb, subband.codeblocks, transformation, segmentationSymbolUsed);
    ...
    var BitModel = (function BitModelClosure() {
        ...
        BitModel.prototype = {
            ...
            decodeSignBit: function BitModel_decodeSignBit(row, column) {
                ...
                var coefficentsMagnitude = this.coefficentsMagnitude;
                var coefficentsSign = this.coefficentsSign;
                var horizontalContribution = calcSignContribution(column > 0 && coefficentsMagnitude[index - 1], coefficentsSign[index - 1], column + 1 < width && coefficentsMagnitude[index + 1], coefficentsSign[index + 1]);
                var verticalContribution = calcSignContribution(row > 0 && coefficentsMagnitude[index - width], coefficentsSign[index - width], row + 1 < height && coefficentsMagnitude[index + width], coefficentsSign[index + width]);
        ...
        Transform.prototype.calculate =
            function transformCalculate(subbands, u0, v0) {
                ...
                for (var i = 1, ii = subbands.length, j = 1; i < ii; i += 3, j++) {
                    ll = this.iterate(ll, subbands[i], subbands[i + 1], subbands[i + 2], u0, v0);
                }
                return ll;
            };
